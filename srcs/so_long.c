...
void	game_init(t_win *so_long)
{
	...
	so_long->win_ptr = mlx_new_window(so_long->mlx_ptr, so_long->map->width ,
			so_long->map->height, "SO LONG");
...
{
	mlx_put_image_to_window(so_long->mlx_ptr, so_long->win_ptr, img, x, y);
}
